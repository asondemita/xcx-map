...
var ExtensionBlocks = /*#__PURE__*/function () {
  ...
  return _createClass(ExtensionBlocks, [{
    ...
    value: function _plotParse(raw) {
      ...
      var splitFields = function splitFields(s) {
        return /[\t,]/.test(s) ? s.split(/[\t,]/) : s.trim().split(/\s+/);
      };
      ...
      if (/[\n;]/.test(text)) {
        ...
        try {
          for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
            ...
            var f = splitFields(row).map(function (s) {
              return s.trim();
            });
      ...
      var tok = splitFields(text).map(function (s) {
        return s.trim();
      });
      var start = 0;
    ...
    value: function scrollBetweenPins(args) {
      ...
      var frame = function frame(t) {
        var p = path(t);
        _this5.centerLng = _this5._worldXToLng(p.x, startZoom);
        _this5.centerLat = _this5._worldYToLat(p.y, startZoom);
        // w = STAGE_WIDTH * 2^(startZoom - z)  ->  z = startZoom - log2(w/STAGE_WIDTH)
        var z = startZoom - Math.log(p.w / STAGE_WIDTH) / Math.LN2;
        return Promise.resolve(_this5._redraw(Math.max(MIN_ZOOM, Math.min(maxZoom, z))));
      };
