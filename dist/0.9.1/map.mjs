...
var ja = {
	...
	"map.setPinName": "ピン [NUMBER] の名前を [NAME] にする",
...
var translations = {
	...
	"map.setPinName": "ピン [NUMBER] のなまえを [NAME] にする",
...
var ExtensionBlocks = /*#__PURE__*/function () {
  ...
  return _createClass(ExtensionBlocks, [{
    ...
    value: function scrollBetweenPins(args) {
      ...
      var dipZoom = Math.min(startZoom, this._fitZoomForBounds(Math.min(from.lat, to.lat), Math.max(from.lat, to.lat), Math.min(from.lng, to.lng), Math.max(from.lng, to.lng)));
      var steps = 24;
      var delayMs = 30;
      var frame = function frame(t) {
        _this5.centerLat = from.lat + (to.lat - from.lat) * t;
        _this5.centerLng = from.lng + (to.lng - from.lng) * t;
        // 4t(1-t) is a parabola: 0 at the ends, 1 at the midpoint.
        var arc = 4 * t * (1 - t);
        _this5.zoom = _this5._clampZoom(startZoom - (startZoom - dipZoom) * arc);
        return Promise.resolve(_this5._redraw());
      };
