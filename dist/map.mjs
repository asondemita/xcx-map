...
var en = {
	...
	"map.setPinComment": "add a [COMMENT] speech bubble to the last pin",
...
var ja = {
	...
	"map.setPinComment": "直前のピンに [COMMENT] のふきだしをつける",
	"map.clearPoints": "ピンを全て消す",
	"map.fitToPoints": "全てのピンが見えるように地図を調整する",
	"map.setPinNumber": "ピンを番号 [MODE] にする",
	"map.pinNumber.off": "無し",
	"map.pinNumber.on": "有り",
	"map.scrollBetweenPins": "ピン [FROM] からピン [TO] へスクロール",
	"map.pinDistance": "ピン [FROM] からピン [TO] までの距離(km)",
	"map.mapLat": "緯度",
	"map.mapLng": "経度",
...
var translations = {
	...
	"map.setPinComment": "ちょくぜんのピンに [COMMENT] のふきだしをつける",
	...
	"map.pinNumber.on": "あり",
	"map.scrollBetweenPins": "ピン [FROM] からピン [TO] へスクロール",
	"map.pinDistance": "ピン [FROM] からピン [TO] までのきょり(km)",
	"map.mapLat": "いど",
	"map.mapLng": "けいど",
...
var ExtensionBlocks = /*#__PURE__*/function () {
  ...
  return _createClass(ExtensionBlocks, [{
    ...
    key: "_drawPin",
    value: function _drawPin(ctx, x, y, color, label, comment) {
      var headR = 6;
      var headCy = y - 15;
    ...
    key: "setPinComment",
    value: function setPinComment(args) {
      if (this._points.length === 0) {
        return;
      }
      var chars = Array.from(Cast.toString(args.COMMENT));
      var text = chars.length > 6 ? "".concat(chars.slice(0, 6).join(''), "...") : chars.join('');
    ...
    value: function scrollBetweenPins(args) {
      ...
      var steps = 24;
      var delayMs = 30;
      var moveTo = function moveTo(t) {
        _this6.centerLat = from.lat + (to.lat - from.lat) * t;
        _this6.centerLng = from.lng + (to.lng - from.lng) * t;
        return Promise.resolve(_this6._redraw());
      };
      var _animate = function animate(i) {
        return moveTo(i / steps).then(function () {
          if (i >= steps) return;
          return new Promise(function (resolve) {
