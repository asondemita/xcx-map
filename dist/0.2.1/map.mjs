...
var en = {
	...
	"map.addPoint": "add point latitude [LAT] longitude [LNG]",
...
var ja = {
	...
	"map.addPoint": "地点 緯度 [LAT] 経度 [LNG] を追加する",
...
var translations = {
	...
	"map.addPoint": "ちてん いど [LAT] けいど [LNG] をついかする",
  ...
  return _createClass(ExtensionBlocks, [{
    ...
    value: function fitToPoints() {
      ...
      var bounds = function bounds(zoom) {
        ...
        var minY = Infinity;
        var maxY = -Infinity;
        var _iterator2 = _createForOfIteratorHelper(_this2._points),
          _step2;
        try {
    ...
    key: "lngToX",
    value: function lngToX(args) {
      var lng = Cast.toNumber(args.LNG);
      var centerWorldX = this._lngToWorldX(this.centerLng, this.zoom);
      return Math.round(this._lngToWorldX(lng, this.zoom) - centerWorldX);
    }
  }, {
    key: "latToY",
    value: function latToY(args) {
      var lat = Cast.toNumber(args.LAT);
      var centerWorldY = this._latToWorldY(this.centerLat, this.zoom);
      return Math.round(centerWorldY - this._latToWorldY(lat, this.zoom));
    }
  }, {
    key: "xToLng",
    value: function xToLng(args) {
