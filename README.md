...
- `地図の中心に () のピンを立てる`（色は赤・黄・青・緑・橙・紫・茶・黒から選択）
- `直前のピンの名前を () にする` / `ピン () の名前を () にする`（地図上に名前を表示。後者はピン番号で指定。7文字以上は6文字＋「...」に短縮、空文字で消去）
- `ピンを全て消す`
- `全てのピンが見えるように地図を調整する`
- `ピンを番号 () にする`（`無し`＝既定／`有り`＝立てた順に 1 から番号を表示）
- `ピン () からピン () へスクロール`（ピン番号で指定。中間でズームアウトして弧を描くように移動し、元のズームに戻る）
- `ピン () からピン () までの距離(km)`（ピン番号で指定。ハバーサイン）
